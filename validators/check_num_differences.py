...
def get_fair_basho(year, month, base_dir="../bashoresultsv2"):
    ...
    while True:
        if idx == 0:
            ...
            idx -= 1
        fair_month = months[idx]
        fair_basho_code = f"{year}{fair_month:02d}"
        csv_path = os.path.join(base_dir, f"{fair_basho_code}.csv")
        if os.path.isfile(csv_path):
            return year, f"{fair_month:02d}"
...
for year in range(1958, 2026):
    for month in basho_months:
        ...
        # find fairious eligible basho (skip cancelled basho files)
        fair = get_fair_basho(year, month)
        if not fair or not all(fair):
            print(f"No fairious basho for {basho_code}")
            continue

        fair_year, fair_month = fair
        fair_basho_code = f"{fair_year}{fair_month}"
        fair_csv_name = f"../fairbanzukeoutputjuryobias/{fair_basho_code}banzuke.csv"
        if not os.path.exists(fair_csv_name):
            continue
        with open(fair_csv_name, newline='', encoding='utf-8') as f:
            fair_rows = list(csv.DictReader(f))
        ...
        for row in fair_rows:
            tags = ["Y", "O", "S", "K"]
            for n in tags:
                if n in row["New Rank Title"]:
                    fair_sanyaku_num += 1
                    break


        # print(real_sanyaku_num, fair_sanyaku_num, basho_code)
        # sanyaku_adj = real_sanyaku_num - fair_sanyaku_num
        ...
        for row in rows:
            ...
            if shouldbreak:
                continue
            # found = False
            for fair_row in fair_rows:
                fair_name = fair_row["Name"]
                if name == fair_name or (name in changed_names and fair_name == changed_names[name]):

                    # out = int(row["Rank"]) - int(fair_row["New Rank"]) - sanyaku_adj
                    out = int(row["Rank"]) - int(fair_row["New Rank"])
                    tourney_differences.append(out)
                    if abs(out) > 12:
                         print(fair_csv_name, csv_name, name)

                    break

        # if intai_wrestlers:
        #     all_tourneys.append(intai_wrestlers)
        used_basho_codes.append(fair_basho_code)
        print(fair_csv_name, csv_name, sum(tourney_differences), tourney_differences )
        differences.append(tourney_differences)
...
print_stats("Heuristic", [sum(n) for n in differences])
