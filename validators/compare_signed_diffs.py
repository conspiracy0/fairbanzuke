...
print_stats("Raw", [sum(n) for n in raw_total_sum])
print()
print_stats("Heuristic", [sum(n) for n in total_sum])
print()
print_stats("Juryo bias", [sum(n) for n in jbiassum])
print()
print_stats("Raw abs ", [sum([abs(x) for x in n]) for n in raw_total_sum])
print()

print_stats("Heuristic abs", [sum([abs(x) for x in n]) for n in total_sum])


print_stats("Juryo bias abs", [sum([abs(x) for x in n]) for n in jbiassum])
...
# Now print stats of stats:
print("Stats of Each Stat Across All Tournaments (Juryo Bias)")
