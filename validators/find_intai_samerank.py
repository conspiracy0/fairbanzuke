    ...
    for month in basho_months:
        ...
        for row in rows:
            ...
            for prow in prev_rows:
                # print(prow)
                # print("$$$")
                # if prow["Name"] == "Annenyama" and name in changed_names:
                #     print("found", name)
                #     # print(changed_names[name])
                #     print(name in changed_names)
                #     time.sleep(1)
                #
                ...
                #     time.sleep(1)
                prev_name = prow["Name"]
                if name == prev_name or (name in changed_names and prev_name == changed_names[name]):
                    # found = True

                    if prow["Wins"] == "0" and (prow["Starting Rank"] == row["Starting Rank"] or int(row["Rank"]) < int(prow["Rank"])-sanyaku_adj ):
                        intai_wrestlers.append([name, row["Starting Rank"], basho_code])
                    # #skip sanyaku and juryo
                    # if "M" not in prow["Starting Rank"]:
                    #     break
        ...
        # used_basho_codes.append(prev_basho_code)


for n in intai_wrestlers:
    print(n)

with open("intai_wrestlers.csv", "w", newline='', encoding="utf-8") as f:
	writer = csv.writer(f)
	writer.writerow(["Name", "Rank", "File"])
	writer.writerows(intai_wrestlers)
